...
all_DJI <- read.csv("Old_Data/all_DJI.csv")

all_SPY <- read.csv("Old_Data/all_SPY.csv")

all_QQQ <- read.csv("Old_Data/all_QQQ.csv")
...
optimal_fun <- function(ER,stocks,n_simulation=1000){
  ...
  mean_ret <- colMeans(price_data[2:5])
  cov_mat <- cov(price_data[2:5]) * 252
...
stock_selected <- c('JETS', 'JETS', 'KO', 'XOM', 'CCL')
